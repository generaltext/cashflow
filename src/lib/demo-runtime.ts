...
// `window.gt` runtime, so the landing page can offer a "try it" demo when the app
// is opened outside General Text (no real runtime injected).
//
// It implements just the GtApi surface Cashflow uses (see src/gt.d.ts), backed by
// plain strings in memory and mirrored to localStorage so edits survive a refresh.
// There's no CRDT and no network — it's single-user, local, throwaway sample data.
// `mode: 'demo'` (with `isLocal: true` as the back-compat signal) makes the data
// layer seed the sample scenarios on first run; `version: 'demo'` lets the app show
// a "Demo" badge. Inert under a real host (we only install it when window.gt is
// absent).

const PREFIX = 'cashflow-demo:'

/** Install the demo runtime as window.gt, unless a real runtime is already present. */
...
function createDemoRuntime(): GtApi {
  ...
  // Restore any prior demo session from localStorage.
  try {
  ...
  return {
    ...
    sync: {
      isLocal: true,
      getFileText: (path) => texts.get(path),
    },
  }
}
